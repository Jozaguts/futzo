...
const {defineField, errors, meta, controlledValues, setFieldError} =
    useForm<LocationStoreRequest>({
      validationSchema: toTypedSchema(
          object({
            name: string()
                .required('El campo es requerido')
                .default(locationStoreRequest.value.name),
            city: string()
                .required('El campo es requerido')
                .default(locationStoreRequest.value.city),
            address: string()
                .required('El campo es requerido')
                .default(locationStoreRequest.value.address),
...
const updateValue = async (value: AutocompletePrediction) => {
  ...
  locationStoreRequest.value.city =
  ...
  <v-container class="pa-0">
    ...
    <v-row>
      ...
      <v-col cols="12" lg="8" md="8">
        <v-text-field :value="name" readonly :error-messages="errors.name">
        </v-text-field>
      </v-col>
    </v-row>
    <v-row>
