...
<template>
  <v-card class="secondary-card" variant="text">
    <v-card-item class="secondary-card-item">
      <v-card-text class="secondary-card__title"
        >Edita tus datos personales</v-card-text
      >
      <v-card-subtitle class="secondary-card__subtitle">
        Estos son tus datos personales, puedes editarlos
        debajo.</v-card-subtitle
      >
    </v-card-item>
    <v-card-text>
      <v-form class="user-data-configuration-form" @submit.prevent="submit">
        <v-row class="row-border-bottom" no-gutters>
          ...
          <v-col cols="4">
            ...
            <small class="text-error">{{
              fields.name.fieldPropsValue["error-messages"][0]
            }}</small>
          </v-col>
        </v-row>
        <v-row class="row-border-bottom" no-gutters>
        ...
        <v-row class="row-border-bottom" no-gutters>
          ...
          <v-col cols="4">
            ...
            ></v-text-field>
            <small class="text-error">{{
              fields.email.fieldPropsValue["error-messages"][0]
            }}</small>
          </v-col>
        </v-row>
        <v-row no-gutters>
