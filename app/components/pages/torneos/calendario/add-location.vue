<script lang="ts" setup>
  ...
  const search = useDebounceFn(async (place: string): Promise<Prediction[]> => {
    ...
    return new Promise((resolve) => {
      autocompleteService.getPlacePredictions({ input: place }, (predictions: Prediction[], status: string) => {
        if (status !== window.google.maps.places.PlacesServiceStatus.OK) {
  ...
  const saveLocationHandler = handleSubmit(async (values) => {
    ...
    await useTournamentStore()
      ...
      .finally(() => {
        tournamentLocationStoreRequest.value = {} as TournamentLocationStoreRequest
...
<template>
  ...
  >
      ...
      <v-container>
        ...
        <v-row>
          ...
          <v-col cols="12" lg="8" md="8">
            ...
            >
              <template #append>
                <v-btn @click="tagHandler" size="small" density="compact" icon>+ </v-btn>
              </template>
            </v-text-field>
    ...
    <template #actions>
      <v-btn block variant="elevated" @click="saveLocationHandler">Crear ubicación </v-btn>
    </template>
  </Dialog>
</template>
