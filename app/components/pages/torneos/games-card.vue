...
<template>
  ...
  <v-card v-else rounded elevation="10" width="100%" height="100%" :max-height="maxHeight" :max-width="maxWidth">
    <v-card-item>
      <v-card-title>
        <span
          class="d-inline-block text-truncate text-subtitle-1"
          :style="[$vuetify.display.mobile ? 'max-width: 180px' : '']"
          >{{ props.title }}</span
        >
      </v-card-title>
      <v-card-subtitle class="text-subtitle-2"
        ><h2 class="text-body-1">{{ tournament?.name }} Jornada 1 de 20</h2></v-card-subtitle
      >
    </v-card-item>
    <v-card-text>
      <v-virtual-scroll :height="scrollHeight" :items="props.items">
