<script lang="ts" setup>
  ...
  async function getFormValues() {
    const formValues = stepRef.value.handleSubmit((values: CreateTournamentForm) => values)
      ...
      <v-container class="pa-0">
        ...
        <v-row>
          <v-col>
            ...
            >
              <BasicInfo ref="stepRef" v-if="steps.current === 'basic-info'" :key="steps.current" />
              <DetailsInfo ref="stepRef" v-if="steps.current === 'details-info'" :key="steps.current" />
            </transition-slide>
          </v-col>
        </v-row>
        <v-row>
