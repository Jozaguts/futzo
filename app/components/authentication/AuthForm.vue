...
<template>
  <div>
    <transition-scale group :axis="{ enter: 'y', leave: 'y' }">
      <v-card
        :key="1"
        ...
        <v-card-text>
          <v-form @submit.prevent="submitHandler" class="px-4">
            <v-row>
              ...
              <v-expand-transition>
                <v-col key="name" cols="12" v-if="showRegisterForm" class="pb-0">
                  ...
                  <div class="pl-2 mt-1" v-auto-animate="{ duration: 100 }">
                    <small v-if="errors?.name" class="d-block text-error">{{ errors?.name }}</small>
                  </div>
                </v-col>
              </v-expand-transition>
              ...
              <v-col cols="12" class="pb-0">
                <div v-if="!showRegisterForm" class="d-flex align-center justify-space-between flex-wrap mt-1 mb-4">
                  <VCheckbox v-model="remember" label="Recuérdame" />
                  <span class="forgot-password" @click="showForgotPassword = !showForgotPassword">
                ...
                <v-expand-transition>
                  <PasswordRules v-model:model-value="password" :show="showRegisterForm && !!password" />
                </v-expand-transition>
