...
export function useProductPrices() {
  ...
  const priceMode = useState<'annually_price' | 'monthly_price'>('productPrices:mode', () => 'annually_price');
  ...
  async function load(force = false) {
    ...
    const p = (async () => {
      try {
        ...
        productPrices.value = await requestFetch<ProductPrices>(
          `${config.public.baseURLBackend}/api/v1/public/products/prices`,
          {
            headers: {
