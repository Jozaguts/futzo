...
export default function (schemaNAme: string, initialValues = {}) {
  ...
  const { defineField, handleSubmit, resetForm, validate, setValues, meta, setFieldValue } = useForm({
  ...
  fields.forEach((field: MaybeRefOrGetter) => {
    const [fieldValue, fieldPropsValue] = defineField(field, vuetifyConfig as any);
...
function getSchemaByName(name: string) {
  ...
  switch (name) {
    ...
    case 'edit-tournament-details-info':
      schemaFields.location_ids = yup.array().of(yup.number()).required(t('forms.required'));
    ...
    case 'create-league':
      ...
      schemaFields.logo = yup.mixed().test('File is required', 'Solo imágenes .jgp, png, svg', (value: any) => {
        if (!value) return true;
        return value?.type?.includes('image/') || typeof value === 'string';
      });
      schemaFields.banner = yup.mixed().test('File is required', 'Solo imágenes .jgp, png, svg', (value: any) => {
        if (!value) return true;
        return value?.type?.includes('image/') || typeof value === 'string';
      });
      schemaFields.status = yupString().nullable();
    ...
    case 'create-coach':
      schemaFields.name = yupString().nullable();
      schemaFields.email = yup.string().email('Correo electrónico no válido').nullable();
      schemaFields.phone = yupString().matches(/^(\+52)?(\d{10})$/, 'Número de teléfono no es válido');
      schemaFields.image = yup
        .mixed()
        .nullable()
        .test('File is required', 'Solo imágenes .jgp, png, svg ', (value: any) => {
    ...
    case 'edit-user':
      schemaFields.name = yupString().required(t('forms.required'));
      schemaFields.email = yup.string().email('Correo electrónico no válido').required(t('forms.required'));
    ...
    case 'edit-password':
      ...
      schemaFields.new_password_confirmation = yup
        .string()
        .required(t('forms.required'))
        .oneOf([yup.ref('new_password'), null as any], 'Las contraseñas no coinciden');
      break;
